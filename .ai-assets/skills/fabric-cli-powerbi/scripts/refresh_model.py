...
def run_fab_command(args: list[str], timeout: int = 60) -> tuple[int, str, str]:
    ...
    try:
        result = subprocess.run(
            ["fab"] + args, capture_output=True, text=True, timeout=timeout
        )
...
def trigger_refresh(
    workspace_id: str, model_id: str, retry_count: int = 1
) -> Dict[str, Any]:
    ...
    if not result["polling_endpoint"]:
        ...
        if request_id:
            result["polling_endpoint"] = (
                f"groups/{workspace_id}/datasets/{model_id}/refreshes/{request_id}"
            )
...
def poll_refresh_status(
    polling_endpoint: str, timeout: int = 300, poll_interval: int = 10
) -> Dict[str, Any]:
    ...
    while True:
        ...
        refresh_status = status_text.get("extendedStatus") or status_text.get(
            "status", "Unknown"
        )
...
def refresh_model(
    model_path: str, wait: bool = False, timeout: int = 300, retry_count: int = 1
) -> Dict[str, Any]:
    ...
    if not workspace or not model:
        ...
        result["message"] = (
            f"Invalid model path: {model_path}. Expected format: Workspace/Model.SemanticModel"
        )
        return result
    ...
    poll_result = poll_refresh_status(
        trigger_result["polling_endpoint"], timeout=timeout
    )
    ...
    if poll_result["status"] == "completed":
        ...
        result["message"] = (
            f"Refresh completed in {poll_result['duration_seconds']} seconds"
        )
    else:
        result["overall_status"] = "failed"
...
Note:
    ...
    parser.add_argument(
        "model", help="Semantic model path (format: Workspace/Model.SemanticModel)"
    )
    parser.add_argument(
        "--wait", "-w", action="store_true", help="Wait for refresh to complete"
    )
    parser.add_argument(
        "--timeout",
        "-t",
    ...
    try:
        ...
        if result.get("poll_result"):
            print(
                f"Duration: {result['poll_result'].get('duration_seconds', 'N/A')} seconds"
            )
