...
def analyze_failure_patterns(history: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for refresh in history:
        ...
        if status in ["completed", "succeeded", "success"]:
            ...
            error_msg = refresh.get(
                "error", refresh.get("errorMessage", "Unknown error")
            )
            if error_msg:
                analysis["common_errors"][error_msg] = (
                    analysis["common_errors"].get(error_msg, 0) + 1
                )
            ...
            if len(analysis["recent_failures"]) < 5:
                analysis["recent_failures"].append(
                    {
                        "time": refresh.get(
                            "startTime", refresh.get("startDateTime", "Unknown")
                        ),
                        "error": error_msg,
                    }
                )
...
def print_refresh_table(history: List[Dict[str, Any]]):
    ...
    for i, refresh in enumerate(history, 1):
        ...
        # Format values
        start_formatted = (
            format_timestamp(start_time)[:22] if start_time != "N/A" else "N/A"
        )
...
def list_refresh_history(
    model_path: str, limit: int = 10, show_details: bool = False
) -> Dict[str, Any]:
    """List refresh history for a semantic model."""

    model_path = ensure_semantic_model_suffix(model_path)

    report = {"model": model_path, "limit": limit, "history": [], "analysis": {}}
...
Output includes:
    ...
    parser.add_argument(
        "model", help="Semantic model path (Workspace/Model.SemanticModel)"
    )
    parser.add_argument(
        "--last",
        "-n",
    ...
    try:
        report = list_refresh_history(
            model_path=args.model, limit=args.last, show_details=args.details
        )
        ...
        if report["analysis"]:
            ...
            if analysis["common_errors"]:
                ...
                for error, count in sorted(
                    analysis["common_errors"].items(), key=lambda x: -x[1]
                )[:3]:
