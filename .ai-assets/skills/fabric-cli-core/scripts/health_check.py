...
def check_cli_installed() -> Dict[str, Any]:
    ...
    if exit_code == 0:
        ...
        check["details"][
            "error"
        ] = "fab CLI not found in PATH. Please install the Fabric CLI."
    else:
        check["status"] = "fail"
...
def check_auth_status() -> Dict[str, Any]:
    ...
    if exit_code == 0:
        ...
        check["status"] = "fail"
        check["details"]["error"] = (
            stderr.strip() or "Not authenticated. Run 'fab auth login' to authenticate."
        )
...
def check_specific_workspace(workspace: str) -> Dict[str, Any]:
    ...
    if exit_code == 0:
        ...
        check["status"] = "fail"
        check["details"]["error"] = (
            stderr.strip() or f"Unable to access workspace '{workspace}'"
        )
...
def print_human_readable(results: Dict[str, Any]) -> None:
    ...
    status_symbols = {"pass": "✓", "fail": "✗", "warn": "⚠", "unknown": "?"}
    ...
    for check in results["checks"]:
        ...
        status_color = {"pass": "", "fail": "", "warn": "", "unknown": ""}.get(
            check["status"], ""
        )
...
Examples:
    ...
    parser.add_argument(
        "--workspace", "-w", help="Specific workspace to verify access (optional)"
    )
    parser.add_argument(
