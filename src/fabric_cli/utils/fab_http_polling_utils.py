...
def get_polling_interval(
    ...
    if custom_polling_interval is not None:
        output_manager.log_debug(f"Using custom polling interval: {custom_polling_interval} seconds")
        return custom_polling_interval
    
    retry_after_value = response_headers.get("retry-after", DEFAULT_POLLING_INTERVAL)
    output_manager.log_debug(f"Retrieved Retry-After header value: {retry_after_value}")
    
    try:
        interval = int(retry_after_value)
        output_manager.log_debug(f"Successfully extracted polling interval: {interval} seconds")
        return interval
    except (ValueError, TypeError):
        output_manager.log_debug(f"Invalid Retry-After header value '{retry_after_value}', cannot convert to integer, using {DEFAULT_POLLING_INTERVAL}-second fallback")
