...
def exec_command(args: Namespace, item: Item) -> None:
    ...
    if response.status_code == 202:
        if args.wait:
            output_manager.print_grey(f"∟ Job instance '{job_instance_id}' created")
            timeout = getattr(args, "timeout", None)
            if timeout is not None:
                output_manager.print_grey(f"∟ Timeout: {timeout} seconds")
            else:
                output_manager.print_grey("∟ Timeout: no timeout specified")
            ...
            try:
                ...
                output_manager.print_warning(str(e))
