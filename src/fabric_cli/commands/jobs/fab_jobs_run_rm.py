...
def exec_command(args: Namespace, context: Item) -> None:
    if not args.force:
        utils_ui.print_warning(
            f"You are about to delete schedule '{args.schedule_id}' from '{context.full_name}'. This action cannot be undone."
        )

    if args.force or utils_ui.prompt_confirm():
        utils_ui.print_grey(
            f"Removing job schedule '{args.schedule_id}'... from '{context.full_name}'"
        )
